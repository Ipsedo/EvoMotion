...
torch::Tensor normal_pdf(const torch::Tensor &x, const torch::Tensor &mu, const torch::Tensor &sigma) {
...
torch::Tensor theta(const torch::Tensor &x) {
    return 0.5f * (1.f + torch::erf(x / std::sqrt(2)));
}
...
torch::Tensor truncated_normal_sample(
    ...
    auto alpha = (min_value - mu) / sigma;
    auto beta = (max_value - mu) / sigma;
    return theta_inv(theta(alpha) + at::rand(mu.sizes(), at::TensorOptions(mu.device())) * (theta(beta) - theta(alpha))) + mu;
}
