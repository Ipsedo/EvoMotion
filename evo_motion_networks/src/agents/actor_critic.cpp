...
a2c_response ActorCriticModule::forward(const torch::Tensor &state) {
    ...
    return {
        ...
        critic->forward(head_out.unsqueeze(0)).squeeze(0)};
}
...
torch::Tensor ActorCritic::act(const torch::Tensor state, const float reward) {
    const a2c_response response = actor_critic->forward(state);
...
void ActorCritic::save(const std::string &output_folder_path) {
    ...
    const auto actor_critic_file = path / "actor_critic.th";
    ...
    // Save networks
    actor_critic->save(actor_critic_archive);
    optimizer->save(optimizer_archive);

    actor_critic_archive.save_to(actor_critic_file);
...
void ActorCritic::load(const std::string &input_folder_path) {
    ...
    actor_critic_archive.load_from(actor_critic_file);
    optimizer_archive.load_from(optimizer_file);

    actor_critic->load(actor_critic_archive);
...
float ActorCritic::grad_norm_mean() {
    ...
    const auto actor_params = actor_critic->parameters();

    for (const auto &p: actor_params) { grad_sum += p.grad().norm().item().toFloat(); }

    return grad_sum / static_cast<float>(actor_params.size());
}