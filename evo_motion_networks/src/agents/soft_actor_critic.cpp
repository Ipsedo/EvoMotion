...
void SoftActorCritic::train(
    ...
    const auto critic_1_loss = torch::mse_loss(
        batched_q_values_1, target_q_values,
        at::Reduction::Mean);
    ...
    const auto critic_2_loss = torch::mse_loss(
        batched_q_values_2, target_q_values,
        at::Reduction::Mean);
    ...
    auto target_v_value =
        (q_value_min - entropy_parameter.alpha() * torch::slice(log_prob.sum(-1), 1, 0, log_prob.size(1) - 1)).detach();
...
void SoftActorCritic::done(torch::Tensor state, float reward) {
    ...
    if (actor->is_training() && static_cast<int>(episodes_buffer.size()) == batch_size) {
        ...
        for (const auto
            ...
            q_values_1_per_episode.push_back(torch::pad(torch::cat(q_value_1_buffer), {0, std::max(pad - 1, 0)}));
            q_values_2_per_episode.push_back(torch::pad(torch::cat(q_value_2_buffer), {0, std::max(pad - 1, 0)}));
