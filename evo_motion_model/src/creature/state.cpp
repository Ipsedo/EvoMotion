...
torch::Tensor ItemProprioceptionState::get_point_state(glm::vec3 point) const {
    glm::vec3 center_pos = state_item.model_matrix() * glm::vec4(glm::vec3(0.f), 1.f);
...
torch::Tensor MemberState::get_state() {
...
torch::Tensor RootMemberState::get_state() {
    ...
    return torch::cat(
        {ItemProprioceptionState::get_state(),
         torch::tensor({log(center_pos.norm()), atan2(center_pos.y(), center_pos.x())})});
}
