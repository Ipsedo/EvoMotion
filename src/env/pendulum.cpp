...
PendulumParams::PendulumParams()
        : pendule_mass(1.f), hinge_push_force(1.f), max_step(1000), reset_nb_frame(2),
...
std::vector<item> PendulumEnv::init_items() {
    ...
    item pendule = create_item_box(glm::vec3(0.f, pendule_pos_y, pendule_pos_z),
                                   glm::mat4(1.f), glm::vec3(pendule_width, pendule_height, pendule_width),
                                   pendule_mass);
...
void PendulumEnv::act(torch::Tensor action) {
    ...
    pendule_rg->applyForce(btVector3(last_action * hinge_push_force, 0, 0), btVector3(0.f, -pendule_height, 0));

    episode_step++;
}

env_step PendulumEnv::compute_new_state() {
    float new_theta = hinge->getHingeAngle();

    new_theta = new_theta > 0 ? float(M_PI) - new_theta : -float(M_PI) - new_theta;

    float cos_theta = cos(new_theta);
    float sin_theta = sin(new_theta);

    // Theta dot : derivative of angular velocity
    float theta_dt = pendule_rg->getAngularVelocity().z();
    float theta_dt_dt = pendule_rg->getAngularFactor().z();
    //float theta_dt = new_theta - theta;

    // last_action * hinge_push_force ?
    //std::cout << theta << " " << pow(theta, 2.) << ", " << theta_dt << " " << pow(theta_dt, 2.) << ", " << last_action << " " << pow(last_action, 2.) << std::endl;
    float reward = -float(pow(new_theta, 2.) + 1e-1 * pow(theta_dt, 2.) + 1e-3 * pow(last_action * 2., 2.));
    ...
    torch::Tensor state = torch::tensor({cos_theta, sin_theta, theta_dt, theta_dt_dt, torque});

    theta = new_theta;

    bool done = episode_step >= max_step;

    return env_step{state, reward, done};
}
...
env_step PendulumEnv::reset_engine() {
    ...
    float torque = rd_uni(rd_gen) * (rd_uni(rd_gen) > 0.5f ? -1.f : 1.f) * 2.f;

    pendule_rg->applyForce(btVector3(torque, 0, 0), btVector3(0.f, -pendule_height, 0));
