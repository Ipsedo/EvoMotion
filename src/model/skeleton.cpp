...
Item Skeleton::get_item(const std::string &name) {
...
JsonMember::JsonMember(const std::string &name, glm::mat4 parent_model_matrix,
                       const nlohmann::json &json_member_input) :
    json_member(json_member_input), name(name),
    model_matrix(parent_model_matrix),
    shape_to_path({
                      {"sphere",   "./resources/obj/sphere.obj"},
                      {"cube",     "./resources/obj/cube.obj"},
                      {"cylinder", "./resources/obj/cylinder.obj"}
                  }),
    member(name + "_" + json_member["name"].get<std::string>(),
           std::make_shared<ObjShape>(
               shape_to_path[json_member["shape"].get<std::string>()]),
           model_matrix *
...
std::vector<std::shared_ptr<AbstractConstraint>> JsonMember::get_children() {
    ...
    std::map<std::string, std::function<std::shared_ptr<AbstractConstraint>(Item, const nlohmann::json &)>> map{
    ...
    for (const auto &child: json_member["children"])
        children.push_back(map.find(child["constraint_type"].get<std::string>())->second(member, child));
...
JsonHingeConstraint::JsonHingeConstraint(Item parent,
                                         const nlohmann::json &json_constraint) {


    child = std::make_shared<JsonMember>(parent,
                                         json_constraint["child_member"]);

    btTransform frame_in_parent = glm_to_bullet(
        json_transformation_to_model_matrix(json_constraint["frame_in_parent"]));
    btTransform frame_in_child = glm_to_bullet(json_transformation_to_model_matrix(json_constraint["frame_in_child"]));
    ...
    hinge_constraint->setLimit(M_PI * json_constraint["limit_degree"]["min"].get<float>() / 180.f,
